...
backlog_trend = pd.DataFrame(columns=["Month", "Backlog"])
...
with tab1:
    ...
    if not priority_df.empty:
        ...
        top_table = priority_view[[c for c in cols if c in priority_view.columns]].head(20).copy()
        ...
        if "Priority Score" in top_table.columns:
...
with tab2:
    ...
    if not mom.empty:
        st.dataframe(mom.round(2), use_container_width=True)
    else:
    ...
    if not trend_month_chart.empty:
        ...
        st.info("No trend chart data available.")

    st.markdown("---")
    st.subheader("Week on Week")
    if not wow.empty:
        st.dataframe(wow.round(2), use_container_width=True)
    else:
        st.info("No week-on-week data available.")
    ...
    if not backlog_trend.empty:
        ...
        st.info("No backlog trend available.")
...
with tab4:
    ...
    if not priority_df.empty:
        ...
        st.dataframe(all_open_view, use_container_width=True)
    else:
...
with tab5:
    ...
    if not ind_monthly.empty:
        ...
        st.dataframe(display_ind_monthly.round(2), use_container_width=True)
    else:
